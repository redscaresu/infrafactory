...
## Next arc candidates (no commitment — pick one based on user intent)

1. **AGENTS.md + README.md optimisation sweep across all 5 repos** (carried directive from 2026-06-05). Single-slice docs arc; canonical pattern is `reference_cross_repo_docs_sweep.md` (4-PR shape). Now that fakegenesys is fully landed, this is the cleanest queued ask.
2. **Pitfall-pruning automation** (shelved 2026-06-06; S107 slot). `docs/plans/pitfall-pruning-automation-plan.md`. Detects pitfalls that haven't fired in N sweeps and demotes them. Lower-impact than (1).
3. **fakegenesys public visibility flip + branch protection** — operator click-ops, not engineering. Listed under "OSS-readiness" pending in STATUS.md.
