...
- **In progress arc**: `docs/plans/fakegenesys-arc-plan.md` — 4th cloud (Genesys Cloud CCaaS) build-out. fakegenesys S108-S119 + S118 merged. Sweep 1 (post-S118 regex fix) landed 40/44; sweep 2 landed 38/44 with 3 transport flakes + 3 persistent genesys failures (architect-flow, full-stack, rbac-and-oauth). **S120 (this PR)**: prompt-level guidance in `prompts/genesys/phase2_generate_hcl.md` teaching the LLM to generate `genesyscloud_flow` YAML files inline via `local_file`. Architect-flow and full-stack scenarios reference `genesyscloud_flow.filepath` but the harness doesn't pre-place YAML files; before this PR the LLM oscillated trying to reference non-existent files. **S121 (next)**: rerun sweep 3 with the prompt guidance.
