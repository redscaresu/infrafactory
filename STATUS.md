...
- **In progress arc**: `docs/plans/fakegenesys-arc-plan.md` — 4th cloud (Genesys Cloud CCaaS) build-out. fakegenesys S108-S113 merged + S114 + S115 cross-link/close-out + S115 hardening. **S116 / S117 TLS MITM (current sub-arc)**: closes the Genesys SDK fidelity gap. fakegenesys#11 added a CONNECT-proxy on `:8443` that MITM-terminates client TLS with leaf certs signed by a boot-time self-signed CA. infrafactory#94 (this PR) fetches the CA at runtime and sets `HTTPS_PROXY` + `SSL_CERT_FILE` + `NO_PROXY` in cloudEnv. macOS one-time keychain step required (see `make fakegenesys-trust-ca-darwin`); Linux/CI is unaffected.
