...
- **In progress arc**: `docs/plans/fakegenesys-arc-plan.md` — 4th cloud (Genesys Cloud CCaaS) build-out. fakegenesys S108-S122d + infrafactory S114-S122 merged. **Sweeps 4 and 5 hit 42 and 43/44** respectively (sweep 5 only fails rbac-and-oauth on a prompt-level LLM HCL mistake). S122a/b/c/d closed mock-gaps revealed at each sweep boundary: group subresources, flow upload-job protocol, datatable+library Division defaults, /tokens/me PascalCase OAuthClient key, /responsemanagement/libraries, /users/me + /users/{id}/roles for the terraform-user role chain. **S122d prompt (this PR)**: teaches LLM that `genesyscloud_user` does NOT accept `roles { }` blocks; use a separate `genesyscloud_user_roles` resource.
