...
- 🎯 **Baseline: 44/44 deterministic** + **fakegenesys v0.2.1** + **27 paired contracts across the family** (CI-enforced via `handlers/contract_audit_test.go` in all 4 siblings) + **ADR-0021 cloud-prefix lockstep CI-enforced** + **standalone smoke harness self-contained** (S136 env-var bridge, 100% green from a fresh clone).
- 🚧 **Arc in progress**: `docs/plans/layer3-real-scaleway-plan.md` (S139–S143) — first real Scaleway apply. **S139, S140 and S142 landed** — B1 shut (sandbox env sealed, endpoint assertion fail-closed), B2 shut (mockway `account/v3` projects with non-empty-delete; verified with a real `tofu apply → plan → destroy` of project+VPC), and expensive resource types denied by default before any API call. **S139–S142 all landed** — the full guardrail set is in place: sealed env + fail-closed endpoint assertion, mockway `account/v3` projects with non-empty-delete, real-API orphan sweep with a state-derived stray check, `AssertProjectDeletable`, interrupt-safe destroy + `infrafactory reap`, a dedicated `infrafactory` fallback project, and a deny-by-default resource-type allowlist. Every guard carries synthetic-drift coverage (break it, watch the test fail). **S143 run 1 (seeded canary) SHIPPED — Layer 3 has now applied to and destroyed real Scaleway infrastructure, verified clean.** Full green: `preflight → init → plan → apply → destroy → orphan_sweep`, with the sweep confirming the run project was destroyed and nothing landed outside it. Independently checked against the account afterwards.

The canary earned its keep immediately by finding two bugs nothing else could: the orphan sweep read `terraform-live.tfstate` *after* destroy had emptied it (so it reported a leak on every successful teardown), and mockway's boot-seeded default project was counted as a Layer 2 orphan — which would have failed `destruction: no_orphans` for **every scenario in the suite** at the next sweep. Both fixed in S143a.

**Next: S143 run 2** — the same scenario through `infrafactory run` with the LLM generating the HCL, proving the full pipeline rather than just the harness. Then arc close-out.
