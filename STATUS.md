...
- **Next arc planned**: `docs/plans/layer3-real-scaleway-plan.md` (S139–S143) — first real Scaleway apply. Layer 3 has been code-complete since S30 and has **never called `api.scaleway.com`**. Two blockers found in planning: the sandbox subprocess inherits `SCW_API_URL` (silent false-green against mockway), and mockway has no `/account/v3/projects` route so the ADR-0010-mandated `scaleway_account_project` 501s at the Layer 2 gate. Arc closes both, adds four guardrails (sealed env, real-API orphan sweep, resource-type allowlist, interrupt-safe destroy), then drives `block-paris` end-to-end against real Scaleway. S143 is operator-gated (needs credentials, spends money).
