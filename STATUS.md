...
- 🚦 **S144 in progress** — `.github/workflows/layer3-gate.yml`: label-triggered (`layer3-gate`) real-cloud PR gate. Applies to Scaleway, destroys, sweeps, and comments the stage summary back on the PR. Same-repo only (fork PRs never see a secret), secrets held by the `layer3` GitHub Environment, head SHA re-verified at run time, and `reap` runs on `always()` so cancellation cannot leak. **This is the check that makes "verified against the real world" true rather than aspirational.**
