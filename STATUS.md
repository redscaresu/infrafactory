...
## Current phase
- **S62 sustain-arc close-out — ADR-0018 codifies N11 retirement criteria + 9 retirements total**. The S54–S62 arc closed: full 39-scenario sweep at 39/39 deterministic (S54), N10 wiring + trim + ratchet fixes (S55), six GCP retirements (S56–S59: firewall + GKE + SQL + GCS + VPC), three cross-cloud retirements (S60: AWS RDS + Scaleway RDB + Scaleway LB), N13 deletion-as-fix extractor (S61), retirement-criteria ADR (S62). GCP phase2 collapsed from 17 prescriptive rules (pre-arc) to 11 (post-arc), or 9 if rules 9 + 12 count as Category-C pending N13 organic validation. ADR-0018 names the three categories: A — redundant (auto-correction carries it); B — replaced by `learned_from_diff` pitfall; C — load-bearing (system, scenario-bound, or no machine-readable signal). The N10→N11→N13 sequence converted hand-written prescription to a self-maintaining artifact across all three cloud providers — strong evidence the architectural shift is general.
- **S61 N13 — deletion-as-fix extractor (`ExtractPrescriptiveAvoid`) landed**. The dual of N10's addition-as-fix: when the LLM clears a failure by REMOVING HCL rather than adding it, the system now emits a "do NOT use" pitfall keyed `source: learned_from_diff_avoid`. Two attribution paths: (a) attribute-level — a removed attribute whose name appears in the failure detail (caught the gcp-cloud-run \"deletion_policy\" hallucination case), (b) resource-level — ALL instances of a resource type removed, with the type name in the failure detail (motivating case: the `google_project_service` and `google_project_iam_member` retirements PR #23 made manually pre-N13). Strict attribution: returns nil when the deletion can't be tied to the failure (unrelated LLM refactors). 4 unit tests pin the four shapes (attribute, resource, unrelated → nil, partial-removal → no resource-type emit). Wired into `run_command.go` alongside the existing addition extractor; both extractors run per cleared-failure iter pair. M91 + S55-T3 ratchets extended to whitelist + size-check the new source. ADR-0012 amended.
- **S60 N11 retirement — generalized across clouds (AWS + Scaleway)**. Retired AWS phase3 rule 3 sub-bullet on RDS `deletion_protection = false`, plus Scaleway phase3 rule 7 (RDB `private_network` ip_net/enable_ipam + LB `assign_flexible_ip` vs `ip_ids` conflict). Re-ran: `aws-rds` → target_reached iter 1 (LLM produced `skip_final_snapshot = true`; provider default `deletion_protection = false` is fine); `mysql-ha-paris` → target_reached iter 2 (RDB `private_network { pn_id = ... enable_ipam = true }` correctly produced); `lb-paris` → target_reached iter 1 (`ip_ids = [scaleway_lb_ip.main.id]` with no `assign_flexible_ip` — correct). Per protocol step 5: rules redundant — delete with no follow-up. Seventh + eighth + ninth N11 retirements (CMEK + firewall + GKE + SQL + GCS + VPC + AWS-RDS + SCW-RDB + SCW-LB). The protocol generalizes to AWS and Scaleway with the same machine-readable-validation-error pattern.
- **S59 N11 retirement — GCP phase2 rule 10 (VPC + subnetwork) retired with existing pitfalls as replacement**. Highest-stakes GCP retirement (affects nearly every networked scenario). Deleted phase2 rule 10 + phase3 rule 5. Re-ran 3 VPC-heavy scenarios: `gcp-vm-network` → target_reached iter 1 (correct explicit `google_compute_network` + `google_compute_subnetwork`, `subnetwork = google_compute_subnetwork.main.id` on instance); `gcp-iam` → target_reached iter 1 (explicit VPC + SA); `gcp-cloud-run` → stuck after 2 iters on an unrelated LLM hallucination (`deletion_policy = "DELETE"` on `google_cloud_run_v2_service`, not a VPC issue). Per protocol step 5: the 2 affected-resource pitfalls already in `pitfalls/gcp.yaml` (`google_compute_instance` VPC + `google_container_cluster` VPC) carry the pattern — rule 10 is redundant given those auto-learned pitfalls. Sixth N11 retirement (CMEK + firewall + GKE + SQL + GCS + VPC). gcp-cloud-run flakiness on `deletion_policy` captured for N12 / mocks audit.
