...
- **S79–S83 arc complete (2026-06-02).** Four PRs landed across two repos (fakeaws#5, infrafactory #58/#59/#60), one documentation slice (S83, this commit). Net deltas:
  ...
  - **S80** — `cmd/s3router/` shim splits S3 traffic between SeaweedFS (data plane) and fakeaws (`?publicAccessBlock` subresource). Architectural correction caught pre-execution: shim, not fakeaws handler. 12 tests; ADR-0015 amended.
  - **S81** — full sweep validated S79+S80 + organically exercised the S78 N3 GCP-escape carve-out. `pitfalls/gcp.yaml` got two new `source: learned` entries on the carve-out resource set (`google_service_account`, `google_service_networking_connection`) — first organic exercise of the carve-out. N13 didn't fire (gcp-full-stack didn't converge through deletion); descriptive `learned` is the right shape here. All sweep pollution discarded per protocol.
  - **S82** — N2 OPA-duplication audit + ratchet. Removed 3 verbatim-OPA-dup pitfalls; `TestPitfallsNoOPADuplication` extracts `msg := sprintf(...)` literals from policies and rejects pitfalls containing ≥30-char chunks. Pitfalls now ratcheted on three orthogonal invariants. ADR-0012 amended.
  - **S83** — Arc close-out (this slice). No new sibling-mock fix: S81 added zero new mock-gaps. gcp-full-stack's persistent failure is a provider-config issue (`service_networking_custom_endpoint` IS injected by `generate_command.go`; the LLM's HCL likely overrides it), filed as next-session work. Full per-slice narrative in `docs/status/ARCHIVE.md` § "2026-06-02 S79–S83".
