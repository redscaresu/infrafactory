...
- **In progress arc**: `docs/plans/fakegenesys-arc-plan.md` — 4th cloud (Genesys Cloud CCaaS) build-out. fakegenesys S108-S117 + S118 (this PR) merged. **S118**: registers `cloudConstraintPolicies["genesys"]` for the 3 policies referenced by all 5 genesys training scenarios (region_restriction, queue_must_have_wrapup, oauth_client_least_privilege). Bundles fakegenesys#13's 10+ read-after-create mock-gap fixes (post-auth SDK probes, OAuth Basic Auth, user/queue subresources, products.total field, queue create=200, queue memberCount derivation). After S118, `genesys-basic-queue` converges target_reached on iteration 1. **S119–S121 (next)**: 3 sustain sweeps for 44/44 deterministic.
