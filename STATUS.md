...
- **In progress arc**: `docs/plans/fakegenesys-arc-plan.md` — 4th cloud (Genesys Cloud CCaaS) build-out. fakegenesys S108-S119 + infrafactory S114-S121 merged. Sweep 3 ceiling was 41/44 with 3 persistent genesys failures. **S122 (this PR)**: unblocks all three. (a) `ensureGenesysFlowAsset` pre-places `flow.yaml` in the generated-files map when the LLM emits a `genesyscloud_flow` resource — the provider reads `filepath` at PLAN time via CustomizeDiff so the file MUST exist before tofu plan; no HCL pattern can satisfy it. (b) Prompt-level guidance for `CLIENT-CREDENTIALS` vs `CLIENT_CREDENTIALS` (LLM kept ignoring the descriptive pitfall). (c) Bundled with fakegenesys#16 which adds the four group subresources the rbac-and-oauth scenario needs (`/individuals`, `/members`, `/voicemail` + `/voicemail/groups/.../policy`).
