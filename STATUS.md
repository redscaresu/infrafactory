...
- **S82 landed (2026-06-02).** OPA-duplication ratchet + 3-entry pitfalls cleanup. Audit found three pitfall rules verbatim-copying OPA policy deny messages (`aws_db_instance.storage_encrypted`, `aws_secretsmanager_secret` AWS-managed KMS, `google_storage_bucket.default_kms_key_name`). All three deleted. `TestPitfallsNoOPADuplication` extracts `msg := sprintf(...)` literals from every `policies/<cloud>/*.rego`, splits on `%s`/`%v`/`%d`/`%t`/`%f`, and rejects any pitfall rule that contains a ≥30-char literal chunk as a substring. Verified retroactively: ratchet correctly fires on all three pre-cleanup entries. ADR-0012 amended.
- **S79 + S80 landed (2026-06-02).** S79: fakeaws KMS tag persistence — fakeaws PR #5 merged, end-to-end validated via aws-full-stack converging target_reached iter 3. S80: `cmd/s3router/` shim landed (PR #59) — splits S3 traffic between SeaweedFS (data plane) and fakeaws (`?publicAccessBlock` subresource). Twelve regression tests cover the routing + fan-out logic. `make mocks-up` now starts the shim on :9091; `infrafactory.yaml` `s3.url` points there. ADR-0015 amended.
