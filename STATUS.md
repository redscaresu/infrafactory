...
- **In progress arc**: `docs/plans/fakegenesys-arc-plan.md` — 4th cloud (Genesys Cloud CCaaS) build-out. fakegenesys S108-S117 + S118 merged. **S118 (this PR)**: closes a S114 oversight in the auto-learning system. `resourceNameRe`, `addressRe`, and `pitfallResourceMatchesCloud` only knew about the three IaaS cloud prefixes (scaleway|google|aws) — every Genesys failure produced an empty Resource, so `ExtractDescriptivePitfall` returned nil and the dynamic loop oscillated for 5 iterations without ever emitting a pitfall. Sweep 1's `genesys-architect-flow` exhausted its repair budget for this reason. Includes regression test `TestExtractResourceFromDetail_Genesys`. **S119–S121 (next)**: 3 sustain sweeps for 44/44 deterministic.
