...
- 🚧 **Arc in progress**: `docs/plans/layer3-real-scaleway-plan.md` (S139–S143) — first real Scaleway apply. **S139, S140 and S142 landed** — B1 shut (sandbox env sealed, endpoint assertion fail-closed), B2 shut (mockway `account/v3` projects with non-empty-delete; verified with a real `tofu apply → plan → destroy` of project+VPC), and expensive resource types denied by default before any API call. **S141 landed in part** — the real-API orphan sweep now backs `no_orphans` for Layer 3 (project-deletion check plus a state-derived check for resources created outside the run project, which a project-scoped sweep cannot see), and `AssertProjectDeletable` refuses any project the run did not create. A dedicated `infrafactory` Scaleway project (`2397e80e-…`) now owns any resource whose HCL omits `project_id`, so a stray is contained somewhere disposable instead of landing in the org default next to `openclaw`. **Outstanding: S141b** (interrupt-safe destroy + `reap` command) — a run killed mid-apply still leaves `terraform-live.tfstate` with nothing to clean it up, so **S143 must not run until S141b lands**. Layer 3 has been code-complete since S30 and has **never called `api.scaleway.com`**. Two blockers found in planning: the sandbox subprocess inherits `SCW_API_URL` (silent false-green against mockway), and mockway has no `/account/v3/projects` route so the ADR-0010-mandated `scaleway_account_project` 501s at the Layer 2 gate. Arc closes both, adds four guardrails (sealed env, real-API orphan sweep, resource-type allowlist, interrupt-safe destroy), then drives `block-paris` end-to-end against real Scaleway. S143 is operator-gated (needs credentials, spends money).
