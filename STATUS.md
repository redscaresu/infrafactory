...
- 🚦 **S144 in progress** — `.github/workflows/layer3-gate.yml`: label-triggered (`layer3-gate`) real-cloud PR gate. Applies to Scaleway, destroys, sweeps, and comments the stage summary back on the PR. Same-repo only (fork PRs never see a secret), secrets held by the `layer3` GitHub Environment, head SHA re-verified at run time, and `reap` runs on `always()` so cancellation cannot leak. **This is the check that makes "verified against the real world" true rather than aspirational.** Proven end-to-end: a labelled PR applied to Scaleway, destroyed, swept clean and commented back in **14s**. Its review also closed a real hole — the deny-by-default allowlist only ran in the generation path, so any route applying pre-existing HCL bypassed it; it now gates every real apply (ADR-0023).
