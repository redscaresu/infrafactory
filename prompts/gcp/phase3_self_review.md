...
4. **API enablement**: Is every required Google API enabled via `google_project_service` BEFORE the resources that need it? Does the first resource of each kind set `depends_on = [google_project_service.NAME]`? Required at minimum: `compute.googleapis.com`, `iam.googleapis.com`, plus `container.googleapis.com` / `sqladmin.googleapis.com` / `storage.googleapis.com` as relevant.
